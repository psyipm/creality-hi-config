...
`python3-virtualenv`) — none of which apply to this OpenWrt build. We do the
agent install by hand: download source, install pure-Python deps with
`pip --target=`. The cfg and init script come from `deploy.sh`.

### One-time bootstrap

**Step A — Install the agent on the printer** (run as root on the printer):

~20 MB of pure-Python deps go to `/mnt/UDISK` (no C toolchain needed;
nothing has native extensions).

```sh
# A1. Download moonraker-obico source (master = 2.2.0+)
...
# A2. Restore the executable bit on bundled scripts (lost in the zip)
find /mnt/UDISK/moonraker-obico/src -name "*.sh" -exec chmod +x {} +

# A3. Install Python deps to a target dir (the system has no `ensurepip`, so a
...
**Step B — Push the cfg and init script** (run from this repo on your local
machine, with `OBICO_URL` set in `.env` and `OBICO_AUTH_TOKEN=` left empty):

```sh
./deploy.sh
```

`deploy.sh` renders `moonraker-obico.cfg.template` (substituting `OBICO_URL`,
empty `OBICO_AUTH_TOKEN`) and uploads it to
`/mnt/UDISK/printer_data/config/moonraker-obico.cfg`, plus the procd init
script. The agent will start but report "not linked" until step C.

**Step C — Link the printer** (run on the printer):
...
The link step prints a 6-digit verification code (e.g. `ntsmq`). On the Obico
web UI, advance the wizard to **Link Printer** and enter the code under
"Switch to manual linking". mDNS auto-discovery is unreliable across Docker
bridges and home subnets, so manual entry is the path that just works.

The link command writes `auth_token = <real_token>` into the printer's cfg
file under `[server]`.

**Step D — Persist the token in `.env`** (run on your local machine):

Read the token from the printer and paste it into `.env`:

```sh
ssh "root@${PRINTER_IP}" "grep '^auth_token' /mnt/UDISK/printer_data/config/moonraker-obico.cfg"
# auth_token = abcdef123456...

# Edit .env, set OBICO_AUTH_TOKEN=abcdef123456...
```

After this, `./deploy.sh` is fully idempotent — the rendered cfg matches the
remote, no upload, no restart. If you skip this step, the next deploy will
hit the safety check and refuse to overwrite the remote cfg (so the printer
stays linked), but you'll see a warning every time.

### Service

`deploy.sh` enables and restarts the procd service whenever the cfg or init
script changes. Manual control:
...
| `moonraker-obico.init` | procd init script for the Obico bridge; deployed to `/etc/init.d/moonraker-obico` |
| `moonraker-obico.cfg.template` | Templated agent config; deployed to `/mnt/UDISK/printer_data/config/moonraker-obico.cfg` with `__OBICO_URL__` and `__OBICO_AUTH_TOKEN__` substituted from `.env`. The agent source + deps are not tracked — see the one-time bootstrap in section 3 |
