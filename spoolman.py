...
class RequestType(_e.Flag):
    ...
    OPTIONS = _e.auto()
class _DummyTracker:
    """Stub for HistoryFieldData.tracker — history component doesn't support auxiliary fields."""
    def update(self, value):
        pass

class HistoryFieldData:
    def __init__(self, name, provider, pretty_name, strategy, reset_callback=None):
        self.name = name
        self.provider = provider
        self.pretty_name = pretty_name
        self.strategy = strategy
        self.reset_callback = reset_callback
        self.has_totals = strategy in ('accumulate', 'collect')
...
class SpoolManager:
    def __init__(self, config: ConfigHelper):
        ...
        if hasattr(history, 'register_auxiliary_field'):
        ...
        if hasattr(announcements, "register_feed"):
    ...
    async def _handle_klippy_ready(self) -> None:
        ...
        try:
            ...
            # Old Moonraker: subscribe_objects does not accept a callback.
            # Fall back to polling for extruder position updates.
            try:
                result = await self.klippy_apis.subscribe_objects(
                ...
                result = {}
            # Cancel any stale poll task, then start a fresh one.
            import asyncio as _asyncio
            _existing = getattr(self, "_poll_task", None)
            if _existing and not _existing.done():
                _existing.cancel()
            self._poll_task = _asyncio.ensure_future(
                self._poll_extruder_position()
            )
    ...
    async def _poll_extruder_position(self) -> None:
        """Poll extruder position at sync_rate_seconds intervals (fallback for
        old Moonraker builds that do not support subscribe_objects callbacks)."""
        import asyncio
        while True:
            await asyncio.sleep(self.sync_rate_seconds)
            try:
                ...
                # Klippy may be disconnected; keep looping and retry next cycle.
                logging.debug(f"spoolman poll: query_objects failed: {e}")
                continue
            if result:
                eventtime = self.eventloop.get_loop_time()
                self._handle_status_update(result, eventtime)
